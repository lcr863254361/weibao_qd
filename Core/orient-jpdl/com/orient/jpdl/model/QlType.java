/*
 * This class was automatically generated with 
 * <a href="http://www.castor.org">Castor 1.3</a>, using an XML
 * Schema.
 * $Id$
 */

package com.orient.jpdl.model;

/**
 * Class QlType.
 * 
 * @version $Revision$ $Date$
 */
@SuppressWarnings("serial")
public class QlType implements java.io.Serializable {


      //--------------------------/
     //- Class/Member Variables -/
    //--------------------------/

    /**
     * Name of the variable in which the result
     *  of the script evaluation will be stored.
     */
    private java.lang.String _var;

    /**
     * Tells whether the query returns a unique result.
     */
    private java.lang.String _unique;

    /**
     * Field _description.
     */
    private java.lang.String _description;

    /**
     * Query text.
     */
    private java.lang.String _query;

    /**
     * Query parameters.
     */
    private com.orient.jpdl.model.Parameters _parameters;


      //----------------/
     //- Constructors -/
    //----------------/

    public QlType() {
        super();
    }


      //-----------/
     //- Methods -/
    //-----------/

    /**
     * Returns the value of field 'description'.
     * 
     * @return the value of field 'Description'.
     */
    public java.lang.String getDescription(
    ) {
        return this._description;
    }

    /**
     * Returns the value of field 'parameters'. The field
     * 'parameters' has the following description: Query
     * parameters.
     * 
     * @return the value of field 'Parameters'.
     */
    public com.orient.jpdl.model.Parameters getParameters(
    ) {
        return this._parameters;
    }

    /**
     * Returns the value of field 'query'. The field 'query' has
     * the following description: Query text.
     * 
     * @return the value of field 'Query'.
     */
    public java.lang.String getQuery(
    ) {
        return this._query;
    }

    /**
     * Returns the value of field 'unique'. The field 'unique' has
     * the following description: Tells whether the query returns a
     * unique result.
     * 
     * @return the value of field 'Unique'.
     */
    public java.lang.String getUnique(
    ) {
        return this._unique;
    }

    /**
     * Returns the value of field 'var'. The field 'var' has the
     * following description: Name of the variable in which the
     * result
     *  of the script evaluation will be stored.
     * 
     * @return the value of field 'Var'.
     */
    public java.lang.String getVar(
    ) {
        return this._var;
    }

    /**
     * Method isValid.
     * 
     * @return true if this object is valid according to the schema
     */
    public boolean isValid(
    ) {
        try {
            validate();
        } catch (org.exolab.castor.xml.ValidationException vex) {
            return false;
        }
        return true;
    }

    /**
     * 
     * 
     * @param out
     * @throws org.exolab.castor.xml.MarshalException if object is
     * null or if any SAXException is thrown during marshaling
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     */
    public void marshal(
            final java.io.Writer out)
    throws org.exolab.castor.xml.MarshalException, org.exolab.castor.xml.ValidationException {
        org.exolab.castor.xml.Marshaller.marshal(this, out);
    }

    /**
     * 
     * 
     * @param handler
     * @throws java.io.IOException if an IOException occurs during
     * marshaling
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     * @throws org.exolab.castor.xml.MarshalException if object is
     * null or if any SAXException is thrown during marshaling
     */
    public void marshal(
            final org.xml.sax.ContentHandler handler)
    throws java.io.IOException, org.exolab.castor.xml.MarshalException, org.exolab.castor.xml.ValidationException {
        org.exolab.castor.xml.Marshaller.marshal(this, handler);
    }

    /**
     * Sets the value of field 'description'.
     * 
     * @param description the value of field 'description'.
     */
    public void setDescription(
            final java.lang.String description) {
        this._description = description;
    }

    /**
     * Sets the value of field 'parameters'. The field 'parameters'
     * has the following description: Query parameters.
     * 
     * @param parameters the value of field 'parameters'.
     */
    public void setParameters(
            final com.orient.jpdl.model.Parameters parameters) {
        this._parameters = parameters;
    }

    /**
     * Sets the value of field 'query'. The field 'query' has the
     * following description: Query text.
     * 
     * @param query the value of field 'query'.
     */
    public void setQuery(
            final java.lang.String query) {
        this._query = query;
    }

    /**
     * Sets the value of field 'unique'. The field 'unique' has the
     * following description: Tells whether the query returns a
     * unique result.
     * 
     * @param unique the value of field 'unique'.
     */
    public void setUnique(
            final java.lang.String unique) {
        this._unique = unique;
    }

    /**
     * Sets the value of field 'var'. The field 'var' has the
     * following description: Name of the variable in which the
     * result
     *  of the script evaluation will be stored.
     * 
     * @param var the value of field 'var'.
     */
    public void setVar(
            final java.lang.String var) {
        this._var = var;
    }

    /**
     * Method unmarshal.
     * 
     * @param reader
     * @throws org.exolab.castor.xml.MarshalException if object is
     * null or if any SAXException is thrown during marshaling
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     * @return the unmarshaled com.orient.jpdl.model.QlType
     */
    public static com.orient.jpdl.model.QlType unmarshal(
            final java.io.Reader reader)
    throws org.exolab.castor.xml.MarshalException, org.exolab.castor.xml.ValidationException {
        return (com.orient.jpdl.model.QlType) org.exolab.castor.xml.Unmarshaller.unmarshal(com.orient.jpdl.model.QlType.class, reader);
    }

    /**
     * 
     * 
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     */
    public void validate(
    )
    throws org.exolab.castor.xml.ValidationException {
        org.exolab.castor.xml.Validator validator = new org.exolab.castor.xml.Validator();
        validator.validate(this);
    }

}
