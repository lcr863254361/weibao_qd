...
public class BeanUtils {
    ...
    static {
        convertUtilsBean.register(new UtilDateConvert(), Date.class);
    ...
    @SuppressWarnings("all")
    public static List<String> getAnnotationClasses(String basePackages, Class annotationClass, String filter) throws IllegalArgumentException {
        ...
        try {
            for (int j = 0; j < arrayPackages.length; j++) {
                ...
                for (int i = 0; i < resources.length; i++) {
                    ...
                    if (annotationTypes.size() > 0 && annotationTypes.contains(annotationClassName)) {
                        String className = metadataReader.getClassMetadata().getClassName();
                        if (StringUtil.isEmpty(filter) ? true : className.contains(filter)) {
                            result.add(className);
                        }
                    }
                }
     ...
     * @throws InvocationTargetException
     */
    public static Object convertMapToJavaBean(Class<?> type, Map<String, Object> map) {
        Object obj = null;
        try {
            ...
            for (int i = 0; i < propertyDescriptors.length; i++) {
                ...
                if (map.containsKey(propertyName)) {
                    ...
                    if (parameterTypes[0].toString().equals(String.class.toString())) {
                        args[0] = value.toString();
                    } else if (parameterTypes[0].toString().equals(Long.class.toString())) {
                        args[0] = Long.parseLong(value.toString());
                    } else if (parameterTypes[0].toString().equals(Integer.class.toString())) {
