package com.orient.utils.ExcelUtil.reader;
...
public class ExcelReader {
     ...
     * @throws Exception
     */
    private void initExcelReader(InputStream inputFile, boolean after2007) throws IOException,
            Exception {
        if (inputFile == null)
    ...
    private List<FieldEntity> getLine(Sheet sheet, int row) {
        ...
        // 遍历所有列
        //增加唯一标志
        FieldEntity IdfieldEntity = new FieldEntity();
        IdfieldEntity.setName("ID");
        IdfieldEntity.setValue(String.valueOf(row));
        IdfieldEntity.setIsKey(FieldEntity.IS_KEY);
        list.add(IdfieldEntity);
        for (int i = colStart; i < filledColumns; i++) {
            // 取得当前单元格
            cell = rowline.getCell(i);
            if (this.excelReaderConfig.getColumns().length > (i - colStart)) {
     ...
     * @throws Exception
     */
    public TableEntity readFile(InputStream input, boolean after2007) throws Exception {
        //读入Excel文件
        this.initExcelReader(input, after2007);
    ...
    public static void main(String[] args) throws Exception {
        ...
        File file = new File("d:\\test2.xls"); // 把test2.xls文件复制到d:
        InputStream input = new FileInputStream(file);

        TableEntity excelEntity = excel.readFile(input, false);
