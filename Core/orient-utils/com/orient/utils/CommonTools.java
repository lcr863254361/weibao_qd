...
public class CommonTools {
     ...
     * 将字符串转化为日期格式
     *
     * @param dateTime 20101115201233[yyyyMMddHHmmss]
     * @return Date
     */
     ...
     * 将字符串转化为日期格式
     *
     * @param dateTime 2010-11-15 20:12:33[yyyy-MM-dd HH:mm:ss]
     * @return Date
     */
    ...
    public static Date timestamp2Date(Object obj) {
        if (obj == null) {
            return null;
        }
        try {
     ...
     *
     * @param date   日期
     * @param format 指定的格式[yyyyMMddHHmmssSSS][yyyy-MM-dd HH:mm:ss]
     * @return
     */
     ...
     * @throws IOException
     */
    public static String saveFile(InputStream stream, String filePath, String fileName) throws FileNotFoundException, IOException {
        if (!FileOperator.isFileExist(filePath)) FileOperator.createFolder(filePath);
        // 完全路径
    ...
    public static String getPreviewImagePath() {
        return getRootPath() + File.separator + "preview" + File.separator + "imagePreview";
    }
    ...
    public static List<Long> stringListToLongList(List<String> list) {
        List<Long> retVal = new ArrayList<>(list.size());
        list.forEach(str -> retVal.add(Long.valueOf(str)));
        return retVal;
    }
    ...
    public static List<String> stringToList(String str, String mid) {
        return stringToList(str, mid, true);

    }

    public static List<String> stringToList(String str, String mid, boolean dropNull) {
        List<String> retVal = new ArrayList<>();
        if (!StringUtil.isEmpty(str)) {
            String[] array = str.split(mid);
            for (String s : array) {
                if (!dropNull || !StringUtil.isEmpty(s)) {
                    retVal.add(s);
                }
            }
        }
    ...
    public static String list2String(List<String> list, String mid) {
        String str = "";
        for (String substr : list) {
            str = str + substr + mid;
        }
    ...
    public static void main(String[] args) {
        ...
        isByteCodeInFlag(1, 3);

        isByteCodeInFlag(1, 6);
        isByteCodeInFlag(2, 3);
        isByteCodeInFlag(2, 6);
    }
    ...
    public static String getRootPath() {
        String result = null;
        try {
            result = CommonTools.class.getResource("/").toURI().getPath().toString();
    ...
    public static <T> List<T> arrayToList(T[] filter) {
    ...
    /**
     * 按位进行计算"与"运算,如果ByteCode在flag中就为true,
     * 例如 byteCode :1 flag: 2(0+2) return false
     * byteCode :1 flag: 3(1+2) return true
     * byteCode :1 flag: 4(0+4) return false
     * byteCode :2 flag: 5(1+4) return false
     * byteCode :2 flag: 6(2+4) return true
     *
     * @param byteCode
     * @param flag
     * @return
     */
    public static boolean isByteCodeInFlag(int byteCode, int flag) {
        int data = byteCode & flag;
        if (data == 0) {
            return false;
        } else {
            return true;
    ...
    public static Map<String, String> MapToStringMap(Map dataMap) {
        Map<String, String> retVal = new HashMap<>();
        dataMap.forEach((key, value) -> {
            retVal.put(String.valueOf(key), String.valueOf(value));
        });
        return retVal;
    }
    ...
    public static <T> T derializeObject(byte[] bytes) {
    ...
    public static <T> boolean hasSameItem(List<T> source, List<T> target) {
        final Boolean[] retVal = {false};
        if (CommonTools.isEmptyList(source) || CommonTools.isEmptyList(target)) {
            return retVal[0];
        } else {
            source.forEach(loopItem -> {
                if (target.contains(loopItem)) {
                    retVal[0] = true;
                }
            });
        }
    ...
    public static <T> List<T> getSortedDataByValue(Map<T, Integer> toSortedData) {
        List<T> retVal = new ArrayList<>();
        Comparator<Map.Entry<T, Integer>> valueComparator = Comparator.comparingInt(Map.Entry::getValue);
        List<Map.Entry<T, Integer>> list = new ArrayList<>(toSortedData.entrySet());
        Collections.sort(list, valueComparator);
        list.forEach(entry -> retVal.add(entry.getKey()));
        return retVal;
    }
}
