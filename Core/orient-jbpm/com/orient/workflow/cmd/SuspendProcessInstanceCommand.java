...
public class SuspendProcessInstanceCommand implements Command<Boolean>{
	...
	public SuspendProcessInstanceCommand(String piId)
	{
		this.piId = piId;
	}
...
//			executionimpl.suspend();
			DbSession dbSession = environment.get(DbSession.class);
			dbSession.cascadeExecutionSuspend(executionimpl);
			ClientExecution ce = dbSession.findExecutionById(execution.getId());
			ce.setState(Execution.STATE_SUSPENDED);
			dbSession.update(ce);
			retVal = true;
		}catch (Exception e) {
			e.printStackTrace();
