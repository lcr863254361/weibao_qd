    ...
    if (Execution.STATE_ACTIVE_ROOT.equals(execution.getState())) {
      ...
      if (isComplete(execution, joinedExecutions)) {
        ...
        if (multiplicity != null) {
          ...
          //获取合并节点的前驱activity
    	  List<? extends Transition> tss = execution.getActivity().getIncomingTransitions(); 
          ...
          for (ExecutionImpl concurrentExecution : concurrentRoot.getExecutions()) {
        	  for(Transition  ts:tss){
        		 if (Execution.STATE_ACTIVE_CONCURRENT.equals(concurrentExecution.getState())
        			&&ts.getSource()==concurrentExecution.getActivity()) {
        				    //将合并节点的前驱activity未执行的execution关闭
