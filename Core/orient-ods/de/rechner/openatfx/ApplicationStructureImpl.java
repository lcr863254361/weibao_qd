...
class ApplicationStructureImpl extends ApplicationStructurePOA {
    ...
    public EnumerationDefinition createEnumerationDefinition(String enumName) throws AoException {
        ...
        for (EnumerationDefinition enumDef : this.enumerationDefinitions) {
            if (enumDef.getName().equals(enumName)) {
                ...
                try {
                    int index = enumDef.getIndex();
                    EnumerationDefinitionImpl enumDefImpl = new EnumerationDefinitionImpl(_this(), index, enumName);
                    EnumerationDefinition newenumDef = EnumerationDefinitionHelper.narrow(modelPOA.servant_to_reference(enumDefImpl));
                    this.enumerationDefinitions.remove(enumDef);
                    this.enumerationDefinitions.add(newenumDef);
                    return newenumDef;
              ...
              // throw new AoException(ErrorCode.AO_BAD_PARAMETER, SeverityFlag.ERROR, 0,"EnumerationDefinition with name '" + enumName + "' already exists");
            }
        }
     ...
     * 
     * @param iter Collection of instances.
