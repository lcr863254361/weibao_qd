        ...
        var retVal = Ext.create('Ext.data.Store', {
            ...
            listeners: {
                ...
                load: function (store, records, successful, eOpts) {
                    ...
                    // var count=0;
                    // for (var i = 0; i < records.length; i++) {
                    //     Ext.each(chosenUsers, function (selected) {
                    //                         //chosenUsers中已经包含
                    //                         if (selected['id']==records[i].raw['id']) {
                    //                             count++;
                    //                         }
                    //                     });
                    //     if (count==0){
                    //         chosenUsers.push(records[i]);
                    //         me.getSelectionModel().select(records[i]);
                    //     }
                    // }
                    ...
                    for (var i = 0; i < records.length; i++) {
                        if (records[i].raw["checked"]) {
                            ...
                            Ext.each(chosenUsers, function (selected) {
                                if (!Ext.isEmpty(chosenUsers[records[i].data.id])||selected['id']==records[i].raw['id']) {
                                    records[i].raw['checked']=true;
                                    arr.push(records[i]);
                                }
                            });
                        }
                    }
                    me.getSelectionModel().select(arr);
                }
            }
        });
        this.store = retVal;
