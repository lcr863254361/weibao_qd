...
Ext.define('OrientTdm.Common.Extend.Grid.OrientModelGrid', {
    ...
    initEvents: function () {
        ...
        me.mon(me, 'afterCreateData', me.afterOperate, me, ['create']);
        me.mon(me, 'afterUpdateData', me.afterOperate, me, ['update']);
        me.mon(me, 'afterDeleteData', me.afterOperate, me, ['delete']);
    },
    doubleClickDetail: function (grid, row) {
        var me = this;
        var selectionModel = grid.getSelectionModel();
        if (selectionModel.mode == 'SIMPLE') {
            selectionModel.clearSelections();
            selectionModel.select(row.index);
        }
        Ext.each(me.down('toolbar').items.items, function (btn) {
            if (btn.text == '详细') {
                this.handler.call(btn, this);
                return false;
            }
        });
    },
