 ...
 */
Ext.define('OrientTdm.DataAnalysis.FormTemplateCheckTree', {
    ...
    initRightPanel: function (record) {
        ...
        if (!Ext.isEmpty(checkTempId)) {
            var formTemplateCenterPanel = me.up('formTemplateMgrDataAnalysisPanel').centerPanel;
            formTemplateCenterPanel.items.each(function (item, index) {
                formTemplateCenterPanel.remove(item);
            });
            var checkItemPanel = Ext.create('OrientTdm.DataAnalysis.FormTemplateHeadCellGrid', {
                region: 'center',
                padding: '0 0 0 5',
                isInst: false,
                isShowProduct: true
