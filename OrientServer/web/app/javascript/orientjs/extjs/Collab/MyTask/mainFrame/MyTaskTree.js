...
Ext.define('OrientTdm.Collab.MyTask.mainFrame.MyTaskTree', {
    ...
    itemClickListener: function (tree, record, item) {
        ...
        } else if (record.get("text")==='计划' && record.parentNode.get("text")==='当前任务') {
            itemToAdd = Ext.create("OrientTdm.Collab.MyTask.plan.PlanDashboard", {
                id: 'planListPanel',
                title: '计划',
                closable: true
            });
        ...
        } else if (record.get("text")==='数据任务' && record.parentNode.get("text")==='当前任务') {
            itemToAdd = Ext.create("OrientTdm.Collab.MyTask.dataTask.DataTaskDashboard", {
                id: 'dataTaskListPanel',
                title: '数据任务',
                closable: true
            });

        } else if (record.get("text")==='历史任务') {
        ...
        } else if (record.get("text")==='计划' && record.parentNode.get("text")==='历史任务') {
            itemToAdd = Ext.create("OrientTdm.Collab.MyTask.historyTask.HistoryTaskDashboard", {
                id: 'hisPlanListPanel',
                title: '历史计划',
                closable: true,
        ...
        } else if (record.get("text")==='数据任务' && record.parentNode.get("text")==='历史任务') {
            itemToAdd = Ext.create("OrientTdm.Collab.MyTask.historyTask.HistoryTaskDashboard", {
                id: 'hisDataTaskListPanel',
                title: '历史数据任务',
                closable: true,
                taskType: 'dataTask'
            });
        ...
        if (centerPanel.contains(itemToAdd)) {
            centerPanel.setActiveTab(itemToAdd);
            return;
        }
