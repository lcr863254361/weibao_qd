...
Ext.define('OrientTdm.TestResourceMgr.GeneralEquipmentMgr.DeviceComponents.UnSelectedDevicePanel', {
    ...
    initComponent: function () {
        ...
        if (me.deviceTypeValue && "-1"!=me.deviceTypeValue) {
            me.customerFilter.push(new CustomerFilter("T_DEV_TYPE_"+TDM_SERVER_CONFIG.DEVICE_SCHEMA_ID+"_ID", CustomerFilter.prototype.SqlOperation.Equal, "", me.deviceTypeValue));
        }
        ...
        Ext.apply(me, {
            listeners: {
                select: function(rowModel, record, index) {
                    ...
                    if(existIds.indexOf(selectedId) < 0) {
    ...
    filterByDevTypeId: function (devTypeId) {
        ...
        if(devTypeId && "-1"!=devTypeId) {
            filter.push(new CustomerFilter("T_DEV_TYPE_"+TDM_SERVER_CONFIG.DEVICE_SCHEMA_ID+"_ID", CustomerFilter.prototype.SqlOperation.In, "", devTypeId));
        }
        for(var i=0; i<me.extraFilters.length; i++) {
