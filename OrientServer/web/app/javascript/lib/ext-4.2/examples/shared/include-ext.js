...
(function () {
    ...
    if (orient) {
        orientPath = (repoDevMode ? path + '/..' : path) +
            '/packages/orient-product-theme/build/orient-product-theme' +
            (repoDevMode ? '-debug' : '') + '.js';

        if (repoDevMode && window.ActiveXObject) {
            Ext = {
                _beforereadyhandler: function () {
                    Ext.Loader.loadScript({url: orientPath});
                }
            };
        } else {
            document.write('<script type="text/javascript" src="' + orientPath + '" defer></script>');
        }
    }
