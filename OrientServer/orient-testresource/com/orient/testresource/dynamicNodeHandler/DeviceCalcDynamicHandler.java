 ...
 */
@RecurrentlyDynamicNode(modelName="T_JLFL", order=3)
public class DeviceCalcDynamicHandler implements BuildRecurrentlyDynamicNode {
    ...
    public List<CustomerFilter> getDynamicFatherNodeFilters(IBusinessModel bm, TBomNode fatherNode) {
        ...
        if(idList!=null && idList.size()>0) {
            ...
            CustomerFilter filter = new CustomerFilter("T_JLFL_"+schemaId+"_ID", EnumInter.SqlOperation.In, CommonTools.list2String(idList));
            retList.add(filter);
        }
        return retList;
    }
    ...
    public List<CustomerFilter> getStaticFatherNodeFilters(IBusinessModel bm, TBomNode fatherNode) {
        ...
        CustomerFilter filter = new CustomerFilter("T_JLFL_"+schemaId+"_ID", EnumInter.SqlOperation.IsNull, "");
        retList.add(filter);
        return retList;
    }
    ...
    public CustomerFilter getCustomerFilter(IBusinessModel refBm, TBomDynamicNode dynamicNode, List<Map<String, Object>> results) {
        ...
        if("T_JLFL".equals(refBm.getName())) {
            filter = new CustomerFilter("T_JLFL_"+schemaId+"_ID", EnumInter.SqlOperation.In, CommonTools.list2String(dataIds));
        }
        else {
