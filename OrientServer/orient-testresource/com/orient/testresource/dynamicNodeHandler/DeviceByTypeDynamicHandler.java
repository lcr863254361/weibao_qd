 ...
 */
@RecurrentlyDynamicNode(modelName="T_DEV_TYPE", order=1)
public class DeviceByTypeDynamicHandler implements BuildRecurrentlyDynamicNode {
    ...
    public List<CustomerFilter> getDynamicFatherNodeFilters(IBusinessModel bm, TBomNode fatherNode) {
        ...
        if(idList!=null && idList.size()>0) {
            ...
            CustomerFilter filter = new CustomerFilter("T_DEV_TYPE_"+schemaId+"_ID", EnumInter.SqlOperation.In, CommonTools.list2String(idList));
            retList.add(filter);
        }
        return retList;
    }
    ...
    public List<CustomerFilter> getStaticFatherNodeFilters(IBusinessModel bm, TBomNode fatherNode) {
        ...
        CustomerFilter filter = new CustomerFilter("T_DEV_TYPE_"+schemaId+"_ID", EnumInter.SqlOperation.IsNull, "");
        retList.add(filter);
        return retList;
    }
    ...
    public String getNodeText(IBusinessModel bm, Map<String, Object> resultNode) {
        return (String) resultNode.get("C_NAME_" + bm.getId());
    }
    ...
    public CustomerFilter getCustomerFilter(IBusinessModel refBm, TBomDynamicNode dynamicNode, List<Map<String, Object>> results) {
        ...
        if("T_DEV_TYPE".equals(refBm.getName())) {
            filter = new CustomerFilter("T_DEV_TYPE_"+schemaId+"_ID", EnumInter.SqlOperation.In, CommonTools.list2String(dataIds));
        }
        else {
